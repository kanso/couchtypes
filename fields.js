/**
 * The building blocks of Types and Forms, Fields help to validate and
 * authorize changes to docuemnt values.
 *
 * @module
 */
...
function (exports, permissions, validators, widgets, utils, _) {
     ...
     * #### Options ####
     *
     * * **omit_empty**  *Boolean* - whether to omit the field from a document
     *                               when the field is empty
     * * **permissions** *Object*  - a permissions check function or an object
     *                               containing separate functions to run on
     *                               create, edit and update operations.
     ...
     * checks the parsed value, so that other field types overridding this
     * method have the raw data available.
     *
    ...
    /**
     * EmbeddedList objects represent multiple instances of a Type embedded
     * within another Type or set of Fields. Its not a true field, but like the
     * Field constructor it acts as a marker when walking through the
     * sub-objects that make up a schema.
     *
     * Exposes the same methods as Field objects.
     *
     * #### Options ####
    ...
    /**
     * Checks for missing or duplicate _ids then runs the type's validate
     * function against each embedded document. Returns an array of validation
     * errors, or an empty array if valid.
     *
     ...
     *
     * To accept an array of values other than strings, add a function to
     * options called parseEach which accepts the string value for each item
    ...
    /**
     * AttachmentField objects are used when constructing content types and
     * forms, and are handled slightly differently than Field objects when
     * parsing requests and validating.
     *
