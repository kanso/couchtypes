...
function (module, exports, events, $, _) {
    ...
    /**
     * Attempts to guess the database name and design doc id from the current
     * URL, or the loc paramter. Returns an object with 'db', 'design_doc' and
     * 'root' properties, or null for a URL not matching the expected format
     * (perhaps behing a vhost).
     *
     * You wouldn't normally use this function directly, but use `db.current()`
     * to return a DB object bound to the current database instead.
     *
     * @name guessCurrent([loc])
     * @param {String} loc - An alternative URL to use instead of
     *      window.location (optional)
    ...
    exports.guessCurrent = function (loc) {
        ...
        /**
         * A database must be named with all lowercase letters (a-z), digits
         * (0-9), or any of the _$()+-/ characters and must end with a slash in
         * the URL. The name has to start with a lowercase letter (a-z).
         *
    ...
    /**
     * Encodes a document id or view, list or show name. This also will make
     * sure the forward-slash is not escaped for documents with id's beginning
     * with "\_design/".
     *
     * @name encode(str)
     ...
     *
     * The DB object also exposes the same module-level methods (eg,
     * createDatabase) so it can be used in-place of the db exports object, for
     * example:
     *
     * ```javascript
    ...
    exports.current = function () {
        ...
        if (!curr) {
            throw new Error(
                'Cannot guess current database URL, if running behind a ' +
                'virtual host you need to explicitly set the database URL ' +
                'using db.use(database_url) instead of db.current()'
            );
        }
    ...
    /**
     * Fetches a spatial view from the database the app is running on. Results
     * are passed to the callback, with the first argument of the callback
     * reserved for any exceptions that occurred (node.js style).
     *
     * __Parameters:__
    ...
    /**
     * Transforms and fetches a document through a show from the database the
     * app is running on. Results are passed to the callback, with the first
     * argument of the callback reserved for any exceptions that occurred
     * (node.js style).
     *
